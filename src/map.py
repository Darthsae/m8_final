...
class RoomPool:
    ...
    def generate(self, map):
        ...
        options = [(spawn_pool, spawn_pool.getScore(room)) for spawn_pool in map.spawn_pool_types.values() if spawn_pool.getScore(room) > 0]
    ...
    def fromDict(cls, game, id, data):
        ...
        room_pool.conditions = [parse(condition, game) for condition in data["conditions"]]
        return room_pool
...
class SpawnPool:
    ...
    def fromDict(cls, game, id, data):
        ...
        spawn_pool.conditions = [parse(condition, game) for condition in data["conditions"]]
        return spawn_pool
...
class RoomInstance:
    ...
    def getDescription(self):
        ...
        if len(self.interactables) > 0:
            to_return += " ".join([interactable.getDescription() for interactable in self.interactables]) + "\n"
        if len(self.entities) > 0:
            to_return += " ".join([entity.getDescription() for entity in self.entities]) + "\n"
        return to_return
...
class Map:
    ...
    def getRoom(self, x, y):
        if not (x, y) in self.__rooms:
            # Generate Room
            options = [(room_pool, room_pool.getScore(self, (x, y))) for room_pool in map(indexOfIndexable(0), self.room_pool_types.values()) if room_pool.getScore(self, (x, y)) > 0]
            if len(options) == 0:
